...
func (s *Store) GetMessageByID(ctx context.Context, msgID int) (models.TgMessage, error) {
	...
	query := `
		SELECT id, date, text, chat_id, first_name, last_name, user_name, from_id, reply_message_id, bot_message_id
		FROM messages
		WHERE id = ?
	`
...
func (s *Store) SaveMessage(ctx context.Context, msg models.TgMessage) error {
	query := `
		INSERT INTO messages (id, date, text, chat_id, first_name, last_name, user_name, from_id, reply_message_id, bot_message_id)
		VALUES (:id, :date, :text, :chat_id, :first_name, :last_name, :user_name, :from_id, :reply_message_id, :bot_message_id)
